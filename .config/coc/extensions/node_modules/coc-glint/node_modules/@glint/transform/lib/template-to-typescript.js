...
exports.templateToTypescript = void 0;
const util_1 = require("./util");
const map_template_contents_1 = require("./map-template-contents");
const scope_stack_1 = __importDefault(require("./scope-stack"));
const SPLATTRIBUTES = '...attributes';
const INLINE_KEYWORDS = ['if', 'yield', 'hash', 'array', 'unless'];
const BLOCK_KEYWORDS = ['if', 'unless'];
/**
 ...
 */
function templateToTypescript(template, { typesPath, globals, meta, typeParams = '', contextType, preamble = [], useJsDoc = false, }) {
    return (0, map_template_contents_1.mapTemplateContents)(template, (ast, { emit, record, rangeForLine, rangeForNode }) => {
        let scope = new scope_stack_1.default([]);
        ...
        function emitTemplateBoilerplate(emitBody) {
            if (meta === null || meta === void 0 ? void 0 : meta.prepend) {
            ...
            if (meta === null || meta === void 0 ? void 0 : meta.append) {
        ...
        function emitInlineKeywordStatement(keyword, node, position) {
            switch (keyword) {
                case 'yield':
                ...
                default:
                    (0, util_1.unreachable)(keyword);
            }
        }
        function emitHashExpression(node) {
            emit.forNode(node, () => {
                (0, util_1.assert)(node.params.length === 0, '{{hash}} only accepts named parameters');
        ...
        function emitArrayExpression(node) {
            emit.forNode(node, () => {
                (0, util_1.assert)(node.hash.pairs.length === 0, '{{array}} only accepts positional parameters');
                emit.text('[');
        ...
        function emitIfExpression(node) {
            emit.forNode(node, () => {
                (0, util_1.assert)(node.params.length >= 2, '{{if}} requires at least two parameters');
                emit.text('(');
        ...
        function emitUnlessExpression(node) {
            emit.forNode(node, () => {
                (0, util_1.assert)(node.params.length >= 2, '{{unless}} requires at least two parameters');
                emit.text('!(');
                emitExpression(node.params[0]);
                emit.text(') ? (');
                emitExpression(node.params[1]);
        ...
        function getInlineKeyword(node) {
                    ...
                    emit.forNode(attr, () => {
                        ...
                        switch (attr.value.type) {
                            ...
                            case 'MustacheStatement':
                                ...
                                break;
                            default:
                                (0, util_1.unreachable)(attr.value);
                        }
                    });
        ...
        function determineBlockChildren(node) {
            ...
            if (named === 0) {
                ...
                for (let child of node.children) {
                    if (!isNamedBlock(child)) {
                        emit.forNode(child, () => (0, util_1.assert)(isAllowedAmongNamedBlocks(child), 'Named blocks may not be mixed with other content'));
                    }
                }
        ...
        function emitSplattributes(node) {
            ...
            if (!splattributes)
                return;
            (0, util_1.assert)(splattributes.value.type === 'TextNode' && splattributes.value.chars === '', '`...attributes` cannot accept a value');
        ...
        function emitMustacheStatement(node, position) {
            ...
            if (keyword) {
                ...
                (0, util_1.assert)(node.params.length === 0 && node.hash.pairs.length === 0, 'Literals do not accept params');
        ...
        function emitYieldStatement(node, position) {
            emit.forNode(node, () => {
                (0, util_1.assert)(position === 'top-level', '{{yield}} may only appear as a top-level statement');
                let to = 'default';
                let toPair = node.hash.pairs.find((pair) => pair.key === 'to');
                if (toPair) {
                    (0, util_1.assert)(toPair.value.type === 'StringLiteral', 'Named block {{yield}}s must have a literal block name');
        ...
        function emitBlockKeywordStatement(keyword, node) {
            switch (keyword) {
                case 'if':
        ...
        function emitIfStatement(node) {
            emit.forNode(node, () => {
                (0, util_1.assert)(node.params.length === 1, '{{#if}} requires exactly one condition');
                emit.text('if (');
                emitExpression(node.params[0]);
                emit.text(') {');
                emit.newline();
        ...
        function emitUnlessStatement(node) {
            emit.forNode(node, () => {
                (0, util_1.assert)(node.params.length === 1, '{{#unless}} requires exactly one condition');
                emit.text('if (!(');
                emitExpression(node.params[0]);
                emit.text(')) {');
                emit.newline();
        ...
        function emitBlockContents(name, nameOffset, blockParams, blockParamsOffset, children) {
            (0, util_1.assert)(blockParams.every((name) => !name.includes('-')), 'Block params must be valid TypeScript identifiers');
...
exports.templateToTypescript = templateToTypescript;
