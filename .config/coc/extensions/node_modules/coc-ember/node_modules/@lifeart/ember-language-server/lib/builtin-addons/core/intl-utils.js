...
async function recursiveIntlTranslationsSearch(server, hashMap, startPath) {
...
function addToHashMap(hash, translationFile, locale, filePath) {
    ...
    Object.keys(items).forEach((p) => {
        ...
        hash[p].push({ locale, text: items[p], location: vscode_languageserver_1.Location.create(uri, range) });
    });
}
