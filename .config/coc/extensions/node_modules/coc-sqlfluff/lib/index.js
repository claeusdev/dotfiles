  ...
  async provideCodeActions(document, range, context) {
    ...
    if (this.lineRange(range) && context.diagnostics.length > 0) {
      ...
      if (line && line.length && !line.startsWith("--")) {
        const edit = import_coc.TextEdit.replace(range, `${line} -- noqa${range.start.line + 1 === range.end.line ? "\n" : ""}`);
        codeActions.push({
        ...
        const disableAllEdit = import_coc.TextEdit.replace(range, `${line} -- noqa: disable=all${range.start.line + 1 === range.end.line ? "\n" : ""}`);
        codeActions.push({
        ...
        const enableAllEdit = import_coc.TextEdit.replace(range, `${line} -- noqa: enable=all${range.start.line + 1 === range.end.line ? "\n" : ""}`);
        codeActions.push({
...
async function doFormat(context, outputChannel, document, range) {
  ...
  if (!toolPath) {
    if (import_fs.default.existsSync(import_path.default.join(context.storagePath, "sqlfluff", "venv", "Scripts", "sqlfluff.exe")) || import_fs.default.existsSync(import_path.default.join(context.storagePath, "sqlfluff", "venv", "bin", "sqlfluff"))) {
      if (process.platform === "win32") {
        toolPath = import_path.default.join(context.storagePath, "sqlfluff", "venv", "Scripts", "sqlfluff.exe");
      } else {
        toolPath = import_path.default.join(context.storagePath, "sqlfluff", "venv", "bin", "sqlfluff");
      }
    } else {
  ...
  args.push("fix");
  args.push("--force");
  const dialect = extensionConfig.get("dialect", "ansi");
  if (dialect) {
...
`);
  ...
  return new Promise((resolve) => {
    let newText = "";
    const cps = import_child_process.default.spawn(toolPath, args, opts);
    cps.on("error", (err) => {
...
`);
        ...
        if (data.toString().match(/Unfixable violations detected/)) {
          ...
          import_coc2.window.showWarningMessage(`sqlfluff: ${JSON.stringify(data.toString().trim())}`);
        }
...
async function sqlfluffInstall(pythonCommand, context) {
  const pathVenv = import_path2.default.join(context.storagePath, "sqlfluff", "venv");
  let pathVenvPython = import_path2.default.join(context.storagePath, "sqlfluff", "venv", "bin", "python");
  if (process.platform === "win32") {
    pathVenvPython = import_path2.default.join(context.storagePath, "sqlfluff", "venv", "Scripts", "python");
  }
  ...
  try {
    import_coc3.window.showInformationMessage(`Install sqlfluff...`);
    await exec(installCmd);
    statusItem.hide();
    import_coc3.window.showInformationMessage(`sqlfluff: installed!`);
  } catch (error) {
    statusItem.hide();
    import_coc3.window.showErrorMessage(`sqlfluff: install failed. | ${error}`);
    throw new Error();
  }
}
...
var LintEngine = class {
  constructor(cmdPath, outputChannel) {
    this.collection = import_coc4.languages.createDiagnosticCollection("sqlfluff");
  ...
  async lint(textDocument) {
    if (!SUPPORT_LANGUAGES.includes(textDocument.languageId))
      return;
    const self = this;
    const filePath = import_coc4.Uri.parse(textDocument.uri).fsPath;
    const args = [];
    const cwd = import_coc4.Uri.file(import_coc4.workspace.root).fsPath;
    const opts = { cwd, shell: true };
    const extensionConfig = import_coc4.workspace.getConfiguration("sqlfluff");
    args.push("lint");
...
`);
    ...
    return new Promise(function(resolve) {
      const cps = import_child_process3.default.spawn(self.cmdPath, args, opts);
        ...
        if (sqlfluffDiagnostics && sqlfluffDiagnostics.length > 0) {
          for (const d of sqlfluffDiagnostics) {
            for (const v of d.violations) {
              ...
              const startPosition = import_coc4.Position.create(lineNo, linePos);
              const endPosition = import_coc4.Position.create(lineNo, linePos);
              diagnostics.push({
                range: import_coc4.Range.create(startPosition, endPosition),
                severity: import_coc4.DiagnosticSeverity.Error,
...
async function installWrapper(pythonCommand, context) {
  ...
  const ret = await import_coc5.window.showPrompt(msg);
  if (ret) {
    try {
